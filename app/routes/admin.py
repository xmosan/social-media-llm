...
<body>
  ...
  <div class="muted">Upload → Generate → Approve → (publishes or schedules depending on your approve route)</div>

  <div class="row" style="margin-top:16px;">
    <div class="card">
      <h2 style="margin-top:0;">1) Upload new post</h2>
      <label>Source text</label>
      <textarea id="source_text" placeholder="Example: short reminder/topic"></textarea>
...
function renderPost(p) {
  const caption = p.caption
    ? `<pre>${esc(p.caption)}${p.hashtags?.length ? "\\n\\n" + esc(p.hashtags.join(" ")) : ""}</pre>`
    : `<div class="muted">No caption yet. Click Generate.</div>`;

  const img = p.media_url
    ? `<a href="${esc(p.media_url)}" target="_blank"><img class="img" src="${esc(p.media_url)}" alt="media"/></a>`
    : "";
